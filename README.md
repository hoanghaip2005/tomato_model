...
- Mỗi box được mô tả bởi:
  ...
  - Công thức tính tọa độ thực:
    ```
    bx = sigmoid(tx) + cx  (cx: cột của cell)
    by = sigmoid(ty) + cy  (cy: hàng của cell)
    bw = exp(tw)
    bh = exp(th)
    ```
...
**Tổng Loss**:
```
Total Loss = λ₁ × CLS_Loss + λ₂ × Box_Loss + λ₃ × OBJ_Loss
```
(λ là các hệ số cân bằng, tự động điều chỉnh theo dataset)
...
6. **Multi-task Loss**:
   ```
   Total Loss = L_RPN_cls + λ₁ × L_RPN_box + L_RCNN_cls + λ₂ × L_RCNN_box
   ```
...
```
Output[i, j] = Σ Σ Σ Input[c, i+m, j+n] × Kernel[c, m, n] + Bias
               c m n
...
```
Output[b, c_out, h, w] = Σ Σ Σ (
                         c_in m n
    Input[b, c_in, h×stride + m, w×stride + n] × 
...
---
Tôi đã tạo file `d:\colab\README.md` chứa giải thích chi tiết. Muốn chỉnh ngôn ngữ hay bổ sung thêm phần nào không?#   t o m a t o _ m o d e l  
 